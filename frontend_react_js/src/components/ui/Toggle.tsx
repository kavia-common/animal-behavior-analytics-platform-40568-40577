  ...
  return (
    <label className="inline-flex items-center gap-2 cursor-pointer">
      <span
        ...
        style={{ backgroundColor: checked ? 'var(--color-primary)' : 'var(--color-neutral-border)' }}
      >
        <input type="checkbox" className="sr-only" checked={checked} onChange={e => onChange(e.target.checked)} />
        <span
          className="absolute left-0.5 top-0.5 w-4 h-4 bg-surface rounded-full transition-transform"
          style={{ transform: `translateX(${checked ? '20px' : '0'})` }}
        />
      </span>
      {label && <span className="text-sm text-secondaryText">{label}</span>}
    </label>
  );
}
